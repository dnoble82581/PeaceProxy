    ...
    public function up(): void
    {
        Schema::create('triggers', function (Blueprint $table) {
            ...
            $table->enum('category', array_map(
                ...
                TriggerCategories::cases()
            ))->default(TriggerCategories::personal); // e.g., Personal, Shared Interest, etc.
            $table->enum('sensitivity_level', array_map(
                fn ($level) => $level->value,
                TriggerSensitivityLevels::cases()
            ))->default(TriggerSensitivityLevels::low->value);
