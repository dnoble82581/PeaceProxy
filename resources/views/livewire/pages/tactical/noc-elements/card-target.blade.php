...
<x-card class="h-full overflow-hidden" header="Target">
    <div class="flex gap-4">
        <div class="h-full flex">
            <div class="grid grid-cols-3 gap-2">
                @foreach(array_slice($imageUrls, 0, 5) as $idx => $url)
                    <img src="{{ $url }}" alt="Subject Image {{ $idx+1 }}" class="w-24 h-24 object-cover rounded-md border border-gray-200 dark:border-dark-500" />
                @endforeach
            </div>
        </div>
