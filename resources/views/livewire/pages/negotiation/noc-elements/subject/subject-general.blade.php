 	...
 	public function loadPrimaryEmailAddress()
 	{
 		try {
 			...
 			if ($contactPoints->isEmpty()) {
 				// Log for debugging
 				logger()->info("No contact points found for subject ID: " . $this->primarySubject->id);
 				return;
 			}
 			...
 			if ($emailContactPoints->isEmpty()) {
 				// Log for debugging
 				logger()->info("No email contact points found for subject ID: " . $this->primarySubject->id);
 				return;
 			}

 			// Try to find a primary email contact point
 			$primaryEmail = $emailContactPoints->firstWhere('is_primary', true);

 			// If no primary email is found, use the first email
 			if (!$primaryEmail) {
 				$primaryEmail = $emailContactPoints->first();
 				// Log for debugging
 				logger()->info("No primary email found, using first email for subject ID: " . $this->primarySubject->id);
 			}

 			// Get the email address from the related ContactEmail model
 			if ($primaryEmail && isset($primaryEmail->email) && $primaryEmail->email) {
 				$this->primaryEmailAddress = $primaryEmail->email->address;
 				// Log for debugging
 				logger()->info("Email address set: " . $this->primaryEmailAddress . " for subject ID: " . $this->primarySubject->id);
 			} else {
 				// Log for debugging
 				logger()->warning("Email relation not found or invalid for contact point ID: " . ($primaryEmail ? $primaryEmail->id : 'null'));
 			}
 		} catch (\Exception $e) {
 			// Log any exceptions
 			logger()->error("Error loading primary email address: " . $e->getMessage());
 			// Don't let exceptions break the page
 		}
 	}
			...
			<div class="relative group">
				...
				<div class="absolute inset-0 bg-black/50 bg-opacity-25 rounded-lg flex items-center justify-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
					...
					<a
							...
							target="_blank"
							onclick="console.log('Email button clicked. Email address: {{ $primaryEmailAddress }}')"
							class="bg-white dark:bg-dark-700 rounded-full p-2 hover:bg-gray-200 dark:hover:bg-dark-600 transition-colors">
						<x-icon
								name="envelope"
								class="size-5 text-primary-500" />
                        ...
                        <div class="bg-white dark:bg-dark-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="sm:flex sm:items-start">
                                <div class="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-blue-100 dark:bg-blue-900 sm:mx-0 sm:h-10 sm:w-10">
                                    <x-icon name="phone" class="h-6 w-6 text-blue-600 dark:text-blue-400" />
                                </div>
                                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                    ...
                                    <div class="mt-2">
                                        <p class="text-sm text-gray-500 dark:text-gray-400">
                                            This functionality is coming soon! We're working hard to bring you direct phone integration in a future update.
                                        </p>
                                    </div>
                                </div>
                            </div>
   ...
   <div class="text-xs flex gap-1">
			...
			<div class="text-xs flex gap-1">
				...
				@if($primaryEmailAddress)
					<span class="text-primary-400">{{ $primaryEmailAddress }}</span>
				@else
					<span>None</span>
				@endif
			</div>
