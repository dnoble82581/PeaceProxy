<?php
	...
	new class extends Component {
		...
		private function fetchSubject($subjectId): ?Subject
		{
			if (empty($subjectId)) {
				return null;
			}
		...
		public function createWarrant(): void
		{
			if (!$this->subject) {
				return;
			}
		...
		public function handleWarrantCreated(array $event): void
		{
			...
			$warrant = app(WarrantFetchingService::class)->getWarrantById($event['warrantId'] ?? null);
		...
		public function handleWarrantUpdated(array $event): void
		{
			if (!$this->subject) {
				return;
			}
		...
		<div class="mt-2 flow-root overflow-hidden rounded-t-lg">
			<div class="">
				<table class="w-full text-left">
					...
					<tbody>
					@forelse(($this->subject?->warrants ?? collect()) as $warrant)
						<tr>
				...
				<x-slot:title>
					<div class="text-center">
						...
						<p class="text-xs">Creating a warrant for:
							<span class="text-primary-400">{{ optional($subject)->name }}</span></p>
					</div>
				</x-slot:title>
				...
				<x-slot:title>
					<div class="text-center">
						...
						<p class="text-xs">
							...
							<span class="text-primary-400">{{ optional($subject)->name }}</span>
						</p>
					</div>
				</x-slot:title>
