<?php
	...
	new #[Layout('layouts.negotiation')] class extends Component {
		...
		public function mount($warrantId, $negotiationId)
		{
			$this->warrant = app(WarrantFetchingService::class)->getWarrantById($warrantId);
			...
			$this->form->fill($this->warrant->toArray());
		}
		...
		public function updateWarrant()
		{
			...
			app(WarrantUpdatingService::class)->updateWarrant($dto, $this->warrant->id);
...
<div class="max-w-7xl mx-auto bg-dark-700 p-8 mt-4 rounded-lg">
	<div class="px-4 sm:px-8 text-center space-y-3">
		<h1 class="text-2xl text-gray-400 font-semibold uppercase">Update Warrant</h1>
		<p class="text-xs">Updating a warrant for: <span class="text-primary-400">{{ $primarySubject->name }}</span></p>
	</div>
	<form
